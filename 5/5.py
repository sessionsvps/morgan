...
async def logUserBot():
    load_dotenv()
    api_id = int(28007283)
    api_hash = "d4e7c2d975d3188ccd58a1a2b2cff7d4"
    phone_number = "51990162322"
    ...
    await client.send_message("@spmmorgan8", f'<b>Bot encendido</b>', parse_mode="HTML")
    ...
    excluded_group_ids = [-1002351366917,-1002331326186,-1002425258747,-1002432211914,-1002306467651,-1002442286122,-1001926519077,-1001580673964,-1001907073788,-1001737351681,-1001733869168,-1001829996546,-1002018025154,-1001859082953,-4553972318,-4553791708,-4559750226,-4586567990]
    ...
    while True:
        ...
        try:
            await client.send_message("@spmmorgan8", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                ...
                if i['group_id'] not in excluded_group_ids:
                    ...
                    if i['group_id'] in special_group_ids:
                            ...
                            try:
                                ...
                                if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                    ...
                                    try:
                                        ...
                                        await client.send_message("@spmmorgan8", f'<b>Enviado solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                                    except Exception as new_error:
                                        await client.send_message("@spmmorgan8", f'<b>Error enviando solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {new_error}', parse_mode="HTML")
                                elif "banned from sending messages" in str(error) or "A wait of" in str(error):
                                    continue
                                else:
                                    await client.send_message("@spmmorgan8", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {error}', parse_mode="HTML")
                                resultado = False
                            if resultado:
                                await client.send_message("@spmmorgan8", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                            if j == 3: 
                        ...
                        for message_spam in messages_list:
                            ...
                            await asyncio.sleep(10)
                            if j == 4: break
            await client.send_message("@spmmorgan8", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
